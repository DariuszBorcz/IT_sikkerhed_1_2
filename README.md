...
4. **Cycle process test**
   ...
   **Flowchart:**
   ```mermaid
   flowchart TD
       A[User anmoder om reset] --> B[System sender token]
       B --> C[User indtaster token + nyt password]
       C --> D[System validerer og opdaterer password]
       D --> E{Password gyldigt?}
       E -->|Ja| F[Opdatering succesfuld]
       E -->|Nej| G[Opdatering fejlede]
   ```
